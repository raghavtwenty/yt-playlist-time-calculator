...
Date Created : Feb 25, 2023 | Last Updated : Feb 27, 2023
Version Info: 1.0
...
yt_api_key = os.getenv('YT_API_YTPLTC')
...
def total_time_cal():
    ...
    try :
        ...
        if (start_vid and end_vid) != None:
            ...
            normal_convert = str(datetime.timedelta(seconds = total_seconds))
            
            # 1.25x
            x125_convert = str(datetime.timedelta(seconds = total_seconds/1.25))

            # 1.5x
            x15_convert = str(datetime.timedelta(seconds = total_seconds/1.5))

            # 1.75x
            x175_convert = str(datetime.timedelta(seconds = total_seconds/1.75))

            # 2.0x
            x2_convert = str(datetime.timedelta(seconds = total_seconds/2))

            # Final Show
            return render_template('calculated_view.html', 
        ...
        # If user enters out of range video value, render invaild html page
        return render_template('invalid_vid_no.html')


# ----- Retrive video details
# Video ask Route
...
def pl_details(pl_id):
    ...
    while True:
        playlist_req = youtube.playlistItems().list(
            part = 'contentDetails',
            playlistId  = pl_id,
            maxResults = 50,    # Max videos that can be retrived 
        ...
        temp_pl_all_vid_id = [pl_res_item["contentDetails"]["videoId"] for pl_res_item in pl_response_items]
...
def main():
    ...
    try:
        ...
        # If not yt link, return invaild page
